...
認証情報の優先順位:
  1. 環境変数 GOOGLE_SERVICE_ACCOUNT_JSON … 鍵JSONの「中身」をそのまま入れる（クラウド運用向け・推奨）
  2. 環境変数 GOOGLE_APPLICATION_CREDENTIALS … 鍵JSONファイルの絶対パス（ローカル運用向け）
...
def load_credentials(scopes=None):
    ...
    raise SystemExit(
        "ERROR: 認証情報が見つかりません。次のどちらかを設定してください:\n"
        "  クラウド運用: 環境変数 GOOGLE_SERVICE_ACCOUNT_JSON に鍵JSONの中身を丸ごと設定\n"
